import { Bot, Clock3, Phone, PhoneForwarded } from "lucide-react";

import { Card } from "@/components/ui/card";
import type { CallsKpi } from "@/data/calls";
...
type CallsKpiCardProps = Readonly<{
  className?: string;
  metric: CallsKpi;
}>;

export function CallsKpiCard({ className, metric }: CallsKpiCardProps) {
  const Icon = icons[metric.icon];

  return (
    <Card
      data-testid={`calls-kpi-${metric.id}`}
