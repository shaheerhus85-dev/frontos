import { BookOpenText, CircleAlert, Gauge, Send } from "lucide-react";

import { Card } from "@/components/ui/card";
import type { KnowledgeKpi } from "@/data/knowledge-base";
...
const tones = {
  ...
  amber: "bg-[#fff7e8] text-[#b87817]",
  violet: "bg-[#f0edff] text-[#7457ff]",
} as const;

export function KnowledgeKpiCard({
  metric,
  className,
}: Readonly<{ metric: KnowledgeKpi; className?: string }>) {
  const Icon = icons[metric.id];
  return (
    <Card
      data-testid={`knowledge-kpi-${metric.id}`}
